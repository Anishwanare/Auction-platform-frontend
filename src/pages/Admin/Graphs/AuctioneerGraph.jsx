...
const AuctioneerGraph = () => {
    const { allUsers } = useSelector((state) => state.SuperAdmin);

    // Preprocess the data to count the number of users registered per month
    const monthlyAuctioneers = Array(12).fill(0); // Initialize an array for 12 months

    const Auctioneers = allUsers.filter((Auctioneer) => {
        return Auctioneer?.role === "Auctioneer"
    })


    Auctioneers?.forEach((user) => {
        const month = new Date(user.createdAt).getMonth(); // Assuming each user has a `createdAt` field
        monthlyAuctioneers[month] += 1; // Increment the count for the corresponding month
    });
    ...
    const data = {
        ...
        datasets: [
            {
                label: 'Total Auctioneers Registered',
...
export default AuctioneerGraph;
